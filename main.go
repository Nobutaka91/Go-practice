...
func main() {
	...
	fmt.Printf("s1: %[1]T %[1]v %v %v\n", s2, len(s2), cap(s2))
	fmt.Println(s1 == nil) // varで初期化したスライスはnilとして認識される
	fmt.Println(s2 == nil) // リテラルで初期化したスライスはnilとして認識されない
	s1 = append(s1, 1, 2, 3)
	fmt.Printf("s1: %[1]T %[1]v %v %v\n", s1, len(s1), cap(s1))
	s3 := []int{4, 5, 6}
	s1 = append(s1, s3...) // スライスを展開して追加
	fmt.Printf("s1: %[1]T %[1]v %v %v\n", s1, len(s1), cap(s1))

	s5 := make([]int, 4, 6) // 長さ4、容量6のスライスを作成
